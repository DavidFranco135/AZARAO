...
export default function DrawLive() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, "raffles", id), (snap) => {
      ...
      const scheduledStr = data.drawScheduledAt
        ? JSON.stringify(data.drawScheduledAt) : null;
  ...
  // Contagem regressiva sincronizada com o criador
...
// ─── Contagem regressiva pública (vista pelos participantes) ─────────────────
function CountdownView({ raffle, seconds }: { raffle: Raffle; seconds: number }) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 pb-10 space-y-8 -mt-8">
        {/* Título */}
        ...
        <AnimatePresence mode="wait">
          <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
            className="text-slate-400 font-medium text-center text-sm max-w-xs"
          >
            {seconds > 45 && "🎯 Prepare-se! O sorteio vai começar em breve..."}
            {seconds > 30 && seconds <= 45 && "🍀 Boa sorte a todos os participantes!"}
            {seconds > 15 && seconds <= 30 && "🎲 Os números estão prestes a ser sorteados!"}
            {seconds > 5  && seconds <= 15 && "⚡ Quase lá! Que a sorte esteja com você!"}
            {seconds <= 5 && "🎰 É agora!!!"}
          </motion.p>
        </AnimatePresence>

        {/* Participantes que compraram */}
        <div className="w-full max-w-sm bg-slate-900 border border-slate-800 rounded-2xl p-4 text-center">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">
            Seus números estão na disputa
          </p>
          <p className="text-sm text-indigo-300 font-medium">
            {raffle.soldNumbers.length} cotas participando do sorteio
          </p>
        </div>
      </div>
    </div>
