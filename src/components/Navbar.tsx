          ...
          <div className="hidden md:flex items-center space-x-5">
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                ...
                {isCreatorOrAdmin && (
                  <>
                    <Link to="/create-raffle" className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2 rounded-lg text-sm font-bold transition-all shadow-lg shadow-indigo-600/20">
                      <PlusCircle size={16} />
                ...
                <div className="flex items-center space-x-3 border-l border-slate-800 pl-4">
                  ...
                  <button onClick={onLogout} className="p-2 text-slate-500 hover:text-red-400 transition-colors rounded-lg hover:bg-slate-900 border border-transparent hover:border-slate-800">
                    <LogOut size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3">
                <Link to="/login" className="text-sm font-semibold text-slate-400 hover:text-white px-4 py-2 transition-colors">Entrar</Link>
                <Link to="/register" className="bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-lg text-sm font-bold shadow-lg shadow-indigo-600/20 transition-all hover:scale-[1.02]">Começar Agora</Link>
              </div>
            )}
          </div>
              ...
              {user ? (
                <>
                  ...
                  <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
                    ...
                    <button onClick={() => { onLogout(); close(); }} className="p-2.5 text-red-400 bg-red-500/10 rounded-xl border border-red-500/10">
                      <LogOut size={18} />
                    </button>
                  </div>
                </>
...
function NavLink({ to, label, icon, active }: { to: string; label: string; icon?: React.ReactNode; active?: boolean }) {
  return (
    <Link to={to} className={`flex items-center gap-1.5 text-sm font-semibold transition-colors ${active ? "text-white" : "text-slate-400 hover:text-white"}`}>
      {icon}
      <span>{label}</span>
    </Link>
  );
...
function MobileLink({ to, label, icon, onClick }: { to: string; label: string; icon?: React.ReactNode; onClick: () => void }) {
  return (
    <Link to={to} onClick={onClick} className="flex items-center space-x-3 text-base font-bold text-slate-300 hover:text-white transition-colors py-1">
